    ...
    while True:
        try:
            valorTransacao = float(
                input("Digite o valor da transação: [Valores nulos não são aceitos]")
            )

            if valorTransacao <= 0:
                print(
                    f"Caro usuário, números neutros são inválidos! Por favor insira um valor maior que zero!"
                )
            else:
...
def tipoTransaction():
    tipoTransacao = str(
        input("Qual o tipo da transação? [Receita| Despesa]").upper()
    ).strip()
...
def category(tipoTransacaoCorrigida):
    if tipoTransacaoCorrigida == "RECEITA":
        ...
        categoriaTransacao = str(
            input(
                "Insira a categoria da transação: SALARIO, VENDAS DE PRODUTOS, PRESTACAO DE SERVICOS, INVESTIMENTOS, REEMBOLSOS, OUTRAS RECEITAS"
            ).upper()
        )
        categoriaTransacaoCorrigida = removerAcento(categoriaTransacao)
        if categoriaTransacaoCorrigida not in categoriasReceitas:
            print(
                "Caro usuário, por favor insira uma categoria de receitas válidas [SALARIO, VENDAS DE PRODUTOS, PRESTACAO DE SERVICOS, INVESTIMENTOS, REEMBOLSOS, OUTRAS RECEITAS]"
            )
            categoriaTransacao = str(
                input(
                    "Insira a categoria da transação: SALARIO, VENDAS DE PRODUTOS, PRESTACAO DE SERVICOS, INVESTIMENTOS, REEMBOLSOS, OUTRAS RECEITAS"
    ...
    if tipoTransacaoCorrigida == "DESPESA":
        ...
        categoriaTransacao = str(
            input(
                "Insira a categoria da transação: [ALIMENTACAO, MORADIA, SAUDE, EDUCACAO LAZER, ROUPAS, HIGIENE PESSOAL, CONTAS, IMPOSTOS, DOACOES]"
            ).upper()
        )
        categoriaTransacaoCorrigida = removerAcento(categoriaTransacao)
        if categoriaTransacaoCorrigida not in categoriasDespesas:
            print(
                "Caro usuário, por favor insira uma categoria de despesas válidas [ALIMENTACAO, MORADIA, TRANSPORTE, SAUDE, EDUCACAO, LAZER, ROUPAS, HIGIENE PESSOAL, CONTAS, IMPOSTOS, DOACOES]"
            )
            categoriaTransacao = str(
                input(
                    "Insira a categoria da transação: [ALIMENTACAO, MORADIA, SAUDE, EDUCACAO, LAZER, ROUPAS, HIGIENE PESSOAL, CONTAS, IMPOSTOS, DOACOES]"
...
def account():  # FUTURAMENTE ADAPTAR PARA CONTAS EMPRESARIAIS
    ...
    contaTransacao = str(
        input(
            "Digite o tipo de sua conta bancária: [CONTA CORRENTE, CONTA POUPANCA, DINHEIRO VIVO, CARTAO DE CREDITO, INVESTIMENTOS, CARTEIRA DIGITAL, OUTROS]"
        ).upper()
    )
    if contaTransacao not in tiposContas:
        print(
            "Caro usuário, por favor insira um tipo de conta válido: [CONTA CORRENTE, CONTA POUPANCA, DINHEIRO VIVO, CARTAO DE CREDITO, INVESTIMENTOS, CARTEIRA DIGITAL, OUTROS]"
        )
        contaTransacao = str(
            input(
                "Digite o tipo de sua conta bancária: [CONTA CORRENTE, CONTA POUPANCA, DINHEIRO VIVO, CARTAO DE CREDITO, INVESTIMENTOS, CARTEIRA DIGITAL, OUTROS]"
...
def tags(
    tipoTransacao, categoriaTransacao
):  # FUTURAMENTE DIVIDIR AS TAGS ENTRE TIPOS DE PESSOAS EX: ADULTOS, CRIANÇAS, ADOLESCENTES
    if tipoTransacao == "DESPESA":
        if categoriaTransacao == "ALIMENTACAO":
            alimentacao = ["SUPERMERCADO", "COMPRA MENSAL", "PROMOCAO"]
            tag = str(
                input(
                    "Adicione uma tag para esta transação: [SUPERMERCADO, COMPRA MENSAL, PROMOCAO]"
                ).upper()
            )
            tagCorrigida = removerAcento(tag)
            if tagCorrigida not in alimentacao:
                print(
                    "Caro usuário, por favor digite uma tag válida para esta categoria"
                )
                tag = str(
                    input(
                        "Adicione uma tag para esta transação: [SUPERMERCADO, COMPRA MENSAL, PROMOCAO]"
        ...
        if categoriaTransacao == "MORADIA":
            ...
            tag = str(
                input(
                    "Adicione uma tag para esta transação: [ALUGUEL, CONDOMINIO, IPTU, SEGURO RESIDENCIAL, FINANCIAMENTO, PRESTACAO DA CASA]"
                ).upper()
            )
            tagCorrigida = removerAcento(tag)
        ...
        if categoriaTransacao == "TRANSPORTE":
            ...
            tag = str(
                input(
                    "Adicione uma tag para esta transação: [COMBUSTIVEL, ESTACIONAMENTO, PEDAGIO, SEGURO VEICULAR, IPVA, LICENCIAMENTO, FINANCIAMENTO DO CARRO]"
                ).upper()
            )
            tagCorrigida = removerAcento(tag)
        ...
        if categoriaTransacao == "EDUCACAO":
            educacao = [
                "MENSALIDADE ESCOLAR",
            ...
            tag = str(
                input(
                    "Adicione uma tag para esta transação: [MENSALIDADE ESCOLAR, MENSALIDADE FACULDADE, CURSO TECNICO, MATERIAL DIDATICO, UNIFORME, TRANSPORTE ESCOLAR, ALIMENTACAO ESCOLAR, TAXA DE MATRICULA, TAXA DE RECUPERACAO, AULAS DE REFORCO, CURSOS ONLINE, CURSOS PROFISSIONALIZANTES, CERTIFICACAO, INSCRICAO EM EVENTOS, MATERIA DO CURSO, PLATAFORMA DE ASSINATURA, LIVROS, REVISTAS ACADEMICAS, EBOOKS, CRECHE, ATIVIDADE EXTRACURRICULAR, PASSEIO ESCOLAR, PALESTRAS, MENTORIA, COACHING "
                ).upper()
            )
            tagCorrigida = removerAcento(tag)
        ...
        if categoriaTransacao == "LAZER":
            ...
            tag = str(
                input(
                    "Adicione uma tag para esta transação: [CINEMA, TEATRO, SHOWS, EVENTOS, MUSEUS, PARQUE, STREAMING, GAMES, RESTAURANTE, BAR, CAFETERIA, DELIVERY, CHURRASCO, FESTAS, VIAGEM, AIRBNB, FOTOGRAFIAS, JARDINAGEM, ANIMAIS DE ESTIMACAO, CLUBE, ACADEMIA]"
                ).upper()
            )
            tagCorrigida = removerAcento(tag)
        ...
        if categoriaTransacao == "ROUPAS":
            ...
            tag = str(
                input(
                    "Adicione uma tag para esta transação: [ROUPAS CASUAL, ROUPAS DE TRABALHO, ROUPAS ESPORTIVAS, ROUPAS DE FESTA, ROUPAS INFANTIS, ROUPAS DE INVERNO, CALCADOS, BOLSAS, CINTOS, RELOGIOS, JOIAS, BONES, OCULOS, LUVAS, LAVANDERIA, COSTURA, TINTURARIA, LIMPEZA DE CALCADOS"
                ).upper()
            )
            tagCorrigida = removerAcento(tag)
        ...
        if categoriaTransacao == "CONTAS":
            ...
            tag = str(
                input(
                    "Adicione uma tag para esta transação: [ENERGIA, AGUA, GAS, INTERNET, CELULAR, ESGOTO, LIXO, ANUIDADE, JUROS, MULTAS, EMPRESTIMO, FINANCIAMENTO, SEGURO, LIMPEZA, SEGURANCA RESIDENCIAL]"
                ).upper()
            )
            tagCorrigida = removerAcento(tag)
        ...
        if categoriaTransacao == "IMPOSTOS":
            ...
            tag = str(
                input(
                    "Adicione uma tag para esta transação: [ITR, IMPOSTO DE RENDA, INSS, CONTRIBUICAO SINDICAL, TAXA AUTONOMO, RETENCAO FONTE, MEI, ICMS, ISS IPI, IOF, TARIFAS FISCAIS]"
                ).upper()
            )
            tagCorrigida = removerAcento(tag)
        ...
        if categoriaTransacao == "DOACOES":
            doacoes = [
                ...
                "PROJETO SOCIAL",
                "ORFANATO",
                "INSTITUICAO DE ANIMAIS",
                "INSITITUICAO DE SAUDE",
                "MEIO AMBIENTE",
                "EDUCACAO",
            ]
            tag = str(
                input(
                    "Adicione uma tag para esta transação: [AJUDA PARA FAMILIA, AJUDA PARA AMIGOS, PRESENTEAMENTOS, SOCORRO EMERGENCIAL, IGREJA, INSTITUICAO DE CARIDADE, PROJETO SOCIAL, ORFANATO, INSTITUICAO DE ANIMAIS, INSTITUICAO DE SAUDE, MEIO AMBIENTE, EDUCACAO]"
                ).upper()
            )
            tagCorrigida = removerAcento(tag)
    ...
    if tipoTransacao == "RECEITA":
        ...
        if categoriaTransacao == "SALARIO":
            ...
            tag = str(
                input("Digite uma tag para esta transação: [EMPRESA, TRABALHO]").upper()
            )
            tagCorrigida = removerAcento(tag)
            if tagCorrigida not in salario:
                print(
                    "Caro usuário, por favor insira uma tag válida para esta categoria"
                )
                tag = str(
                    input(
                        "Digite uma tag para esta transação: [EMPRESA, TRABALHO]"
                    ).upper()
                )
                tagCorrigida = removerAcento(tag)
                return tagCorrigida
        ...
        if categoriaTransacao == "VENDA DE PRODUTOS":
            ...
            tag = str(
                input(
                    "Digite uma tag para esta transação: [ALIMENTOS, ROUPAS, ARTESANATO , ELETRONICOS ,COSMETICOS , ACESSORIOS, DIGITAL]"
                ).upper()
            )
            tagCorrigida = removerAcento(tag)
        ...
        if categoriaTransacao == "PRESTACAO DE SERVICOS":
            ...
            tag = str(
                input(
                    "Digite uma tag para esta transação: [CONSULTORIA, DESIGN, MANUTENCAO, AULAS, BELEZA, TRANSPORTE, FREELANCER, SUPORTE, ENTREGA]"
                ).upper()
            )
            tagCorrigida = removerAcento(tag)
            if tagCorrigida not in prestacao:
                ...
                tag = str(
                    input(
                        "Digite uma tag para esta transação: [CONSULTORIA, DESIGN, MANUTENÇÃO, AULAS, BELEZA, TRANSPORTE, FREELANCER, SUPORTE, ENTREGA]"
                    ).upper()
                )
                tagCorrigida = removerAcento(tag)
                return tagCorrigida
        ...
        if categoriaTransacao == "INVESTIMENTOS":
            ...
            tag = str(
                input(
                    "Digite uma tag para esta transação: [RENDA FIXA, RENDA VARIAVEL, TESOURO DIRETO, ACOES, FUNDOS IMOBILIARIOS, CRIPTO, CDB, POUPANCA, ETFs]"
                ).upper()
            )
            tagCorrigida = removerAcento(tag)
        ...
        if categoriaTransacao == "REEMBOLSOS":
            ...
            tag = str(
                input(
                    "Digite uma tag para esta transação: [EMPRESA, PLANO DE SAUDE, CLIENTE, AMIGO, GOVERNO, LOJA, BANCARIO]"
                ).upper()
            )
            tagCorrigida = removerAcento(tag)
